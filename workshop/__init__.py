...
def run():
    ...
    parser = argparse.ArgumentParser(description ='This is a test parser')
    parser.add_argument('--display', help='Prints a statement',default='No display message')
    parser.add_argument('--config', help='Prints a config.ini file', default = False)
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
...
##python workshop/ --config ./workshop/config.ini
